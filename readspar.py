...
INPUT_LOANS_FILE = "loans.parquet"
...
def load_loans(
    ...
    try:
        ...
        raise ModuleNotFoundError(
            "pandas is required to read Parquet files. Install with: pip install pandas pyarrow"
        ) from exc

    lower_path = file_path.lower()
    if not lower_path.endswith(".parquet"):
    ...
    for _, row in df.iterrows():
        ...
        loans.append(
            {
                "loan_number": as_text(get_value("loan_number")),
                "periods_months": int(as_text(get_value("periods_months"))),
                "projected_close_date": parse_date(
                    as_text(get_value("projected_close_date"))
                ),
                "interest_start_date": parse_date(
                    as_text(get_value("interest_start_date"))
                ),
                "first_payment_date": parse_date(
                    as_text(get_value("first_payment_date"))
                ),
                "cycle_day": int(as_text(get_value("cycle_day"))),
                "annual_rate_percent": Decimal(
                    as_text(get_value("annual_rate_percent"))
                ),
                "loan_amount": Decimal(as_text(get_value("loan_amount"))),
                "monthly_payment": Decimal(
                    as_text(get_value("monthly_payment"))
                ),
            }
        )
    return loans
